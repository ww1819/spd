...
CREATE TABLE IF NOT EXISTS `fd_department` (
  ...
  `his_id` varchar(128) DEFAULT NULL COMMENT 'HIS科室编码',
  `campus` varchar(128) DEFAULT NULL COMMENT '院区',
  `status` char(1) DEFAULT '1' COMMENT '启用状态（字典 is_use_status：1启用 2停用）',
...
/
/* 科室申购（按仓库；可由汇总申购审核按仓拆分生成，见 dep_purchase_apply_agg） */
/
CREATE TABLE IF NOT EXISTS `dep_purchase_apply` (
  ...
  `is_gz` char(1) DEFAULT NULL COMMENT '高低值标志（1高值 2低值）',
...
CREATE TABLE IF NOT EXISTS `gz_order_entry_inhospitalcode_list` (
  ...
  `warehouse_id` bigint DEFAULT NULL COMMENT '仓库ID',
  `supplier_id` bigint DEFAULT NULL COMMENT '供应商ID',
  `material_name` varchar(256) DEFAULT NULL COMMENT '产品名称快照（备货验收直打）',
  `material_speci` varchar(256) DEFAULT NULL COMMENT '规格快照',
  `material_model` varchar(256) DEFAULT NULL COMMENT '型号快照',
  `material_unit_name` varchar(64) DEFAULT NULL COMMENT '单位名称快照',
  `factory_id` bigint DEFAULT NULL COMMENT '生产厂家ID快照',
  `factory_name` varchar(256) DEFAULT NULL COMMENT '生产厂家名称快照',
  `supplier_name` varchar(256) DEFAULT NULL COMMENT '供应商名称快照',
  `warehouse_name` varchar(200) DEFAULT NULL COMMENT '仓库名称快照',
  `finance_category_name` varchar(200) DEFAULT NULL COMMENT '财务分类名称快照',
  `register_no` varchar(128) DEFAULT NULL COMMENT '注册证号快照',
  `brand_name` varchar(128) DEFAULT NULL COMMENT '品牌快照',
  `hc_barcode_master_id` varchar(36) DEFAULT NULL COMMENT '归属主档 hc_barcode_master.id',
  `del_flag` int NOT NULL DEFAULT 0 COMMENT '删除标志',
...
CREATE TABLE IF NOT EXISTS `gz_traceability` (
  ...
  `apply_dept_id` bigint DEFAULT NULL COMMENT '申请科室ID',
  `exec_dept_id` bigint DEFAULT NULL COMMENT '执行科室ID',
  `write_off_dept_id` bigint DEFAULT NULL COMMENT '核销科室ID（HIS镜像高值扫码实际扣减科室）',
...
CREATE TABLE IF NOT EXISTS `gz_depot_inventory` (
  ...
  `master_barcode` varchar(200) DEFAULT NULL COMMENT '主条码',
  `secondary_barcode` varchar(200) DEFAULT NULL COMMENT '辅条码',
  `material_name` varchar(256) DEFAULT NULL COMMENT '产品名称快照（直打条码）',
  `material_speci` varchar(256) DEFAULT NULL COMMENT '规格快照',
  `material_model` varchar(256) DEFAULT NULL COMMENT '型号快照',
  `factory_name` varchar(256) DEFAULT NULL COMMENT '生产厂家名称快照',
  `supplier_name` varchar(256) DEFAULT NULL COMMENT '供应商名称快照',
...
CREATE TABLE IF NOT EXISTS `stk_io_bill` (
  ...
  `delivery_ref_warehouse_id` varchar(128) DEFAULT NULL COMMENT '引用配送单入库：仓库ID快照',
  `delivery_ref_warehouse_name` varchar(256) DEFAULT NULL COMMENT '引用配送单入库：仓库名称快照',
  `delivery_ref_supplier_id` varchar(128) DEFAULT NULL COMMENT '引用配送单入库：供应商ID快照',
...
CREATE TABLE IF NOT EXISTS `stk_inventory` (
  ...
  `third_party_batch_no` varchar(128) DEFAULT NULL COMMENT '第三方系统批次号',
  `batch_number` varchar(100) DEFAULT NULL COMMENT '批号',
  `tenant_id` varchar(36) DEFAULT NULL COMMENT '租户ID(同sb_customer.customer_id)',
  `main_barcode` varchar(128) DEFAULT NULL COMMENT '高值耗材主条码',
  `sub_barcode` varchar(128) DEFAULT NULL COMMENT '高值耗材辅条码',
  ...
  `material_model` varchar(256) DEFAULT NULL COMMENT '型号（快照）',
  `material_factory_id` bigint DEFAULT NULL COMMENT '生产厂家ID（快照，fd_factory.factory_id）',
  `lv_inhospital_package_code` varchar(200) DEFAULT NULL COMMENT '低值定数包院内码（入库审核生成，与条码明细一致）',
  `create_by` varchar(64) DEFAULT NULL COMMENT '创建者',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT NULL COMMENT '更新者',
  `update_time` datetime DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS `stk_dep_inventory` (
  ...
  `material_model` varchar(256) DEFAULT NULL COMMENT '型号（快照）',
  `material_factory_id` bigint DEFAULT NULL COMMENT '生产厂家ID（快照，fd_factory.factory_id）',
  `lv_inhospital_package_code` varchar(200) DEFAULT NULL COMMENT '低值定数包院内码（出库审核生成）',
  `create_by` varchar(64) DEFAULT NULL COMMENT '创建者',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT NULL COMMENT '更新者',
  `update_time` datetime DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS `stk_io_stocktaking_entry` (
  ...
  `material_id` bigint DEFAULT NULL COMMENT '耗材ID',
  `unit_price` decimal(18,2) DEFAULT NULL COMMENT '单价',
  `qty` decimal(18,2) DEFAULT NULL COMMENT '账目数量',
  `price` decimal(18,2) DEFAULT NULL COMMENT '价格',
  `amt` decimal(18,2) DEFAULT NULL COMMENT '金额',
  `batch_no` varchar(100) DEFAULT NULL COMMENT '批次号',
  ...
  `del_flag` int NOT NULL DEFAULT 0 COMMENT '删除标志',
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
  `stock_qty` decimal(18,2) DEFAULT NULL COMMENT '实盘数量',
  ...
  `his_id` varchar(128) DEFAULT NULL COMMENT '第三方/HIS系统库存明细ID',
  `third_party_batch_no` varchar(128) DEFAULT NULL COMMENT '第三方系统批次号',
  `tenant_id` varchar(36) DEFAULT NULL COMMENT '租户ID',
  `delete_by` varchar(64) DEFAULT NULL COMMENT '删除者',
  `delete_time` datetime DEFAULT NULL COMMENT '删除时间',
...
CREATE TABLE IF NOT EXISTS `his_inpatient_charge_mirror` (
  ...
  `dept_code` varchar(32) DEFAULT NULL COMMENT '开单科室编码',
  `dept_name` varchar(128) DEFAULT NULL COMMENT '开单科室名称',
  `exec_dept_id` varchar(12) DEFAULT NULL COMMENT '执行科室ID(HIS)',
  `exec_dept_name` varchar(32) DEFAULT NULL COMMENT '执行科室名称(HIS)',
...
CREATE TABLE IF NOT EXISTS `his_outpatient_charge_mirror` (
  ...
  `clinic_code` varchar(32) DEFAULT NULL COMMENT '开单科室编码(就诊编码)',
  `clinic_name` varchar(128) DEFAULT NULL COMMENT '开单科室名称(就诊名称)',
  `exec_dept_id` varchar(12) DEFAULT NULL COMMENT '执行科室ID(HIS)',
  `exec_dept_name` varchar(32) DEFAULT NULL COMMENT '执行科室名称(HIS)',
  `doctor_id` varchar(32) DEFAULT NULL COMMENT '医生ID',
  `doctor_name` varchar(128) DEFAULT NULL COMMENT '医生姓名',
...
CREATE TABLE IF NOT EXISTS `his_patient_charge_mirror_unified` (
  ...
  `dept_code` varchar(32) DEFAULT NULL COMMENT '住院开单科室编码',
  `dept_name` varchar(128) DEFAULT NULL COMMENT '住院开单科室名称',
  `clinic_code` varchar(32) DEFAULT NULL COMMENT '门诊开单科室编码',
  `clinic_name` varchar(128) DEFAULT NULL COMMENT '门诊开单科室名称',
  `exec_dept_id` varchar(12) DEFAULT NULL COMMENT '执行科室ID(HIS)',
  `exec_dept_name` varchar(32) DEFAULT NULL COMMENT '执行科室名称(HIS)',
...
CREATE TABLE IF NOT EXISTS `his_mirror_consume_link` (
  ...
  `write_off_dept_id` bigint DEFAULT NULL COMMENT '核销科室ID（用户所选实际扣减科室）',
  ...
  KEY `idx_hmcl_confirm` (`tenant_id`,`confirm_status`)
...
CREATE TABLE IF NOT EXISTS `gz_high_consume_confirm` (
  ...
  `period_end` datetime DEFAULT NULL COMMENT '勾选明细核销时间范围止',
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
...
-- ========== 低值定数包/院内码、高低值条码归属与流通、高值仓库/科室流水（主键 UUID7 字符串；业务外键逻辑 varchar） ==========

-- 低值：入库审核/出库审核生成的定数包院内码明细（入、出共用一张表，用 io_direction 区分）
CREATE TABLE IF NOT EXISTS `stk_lv_io_inhospital_barcode` (
  `id` varchar(36) NOT NULL COMMENT '主键UUID7（36位含连字符）',
  `tenant_id` varchar(36) NOT NULL COMMENT '租户ID',
  `io_direction` tinyint NOT NULL COMMENT '1入库审核生成 2出库审核生成',
  `stk_io_bill_id` varchar(64) NOT NULL COMMENT '出入库主表ID(stk_io_bill.id 字符形式)',
  `stk_io_bill_no` varchar(128) DEFAULT NULL COMMENT '单号快照',
  `stk_io_bill_type` int DEFAULT NULL COMMENT '单据类型快照 101/201/301/401 等',
  `stk_io_bill_entry_id` varchar(64) DEFAULT NULL COMMENT '出入库明细ID(stk_io_bill_entry.id 字符形式)',
  `warehouse_id` varchar(64) DEFAULT NULL COMMENT '仓库ID',
  `warehouse_code` varchar(64) DEFAULT NULL COMMENT '仓库编码快照',
  `warehouse_name` varchar(200) DEFAULT NULL COMMENT '仓库名称快照',
  `department_id` varchar(64) DEFAULT NULL COMMENT '科室ID',
  `department_code` varchar(64) DEFAULT NULL COMMENT '科室编码快照',
  `department_name` varchar(200) DEFAULT NULL COMMENT '科室名称快照',
  `material_id` varchar(64) DEFAULT NULL COMMENT '耗材档案ID',
  `material_code` varchar(128) DEFAULT NULL COMMENT '耗材编码快照',
  `material_name` varchar(256) DEFAULT NULL COMMENT '产品名称快照',
  `material_speci` varchar(256) DEFAULT NULL COMMENT '规格快照',
  `material_model` varchar(256) DEFAULT NULL COMMENT '型号快照',
  `material_unit_name` varchar(64) DEFAULT NULL COMMENT '单位名称快照',
  `factory_id` varchar(64) DEFAULT NULL COMMENT '生产厂家ID快照',
  `factory_name` varchar(256) DEFAULT NULL COMMENT '生产厂家名称快照',
  `supplier_id` varchar(64) DEFAULT NULL COMMENT '供应商ID快照',
  `supplier_name` varchar(256) DEFAULT NULL COMMENT '供应商名称快照',
  `batch_no` varchar(100) DEFAULT NULL COMMENT '批次号',
  `batch_number` varchar(100) DEFAULT NULL COMMENT '批号',
  `begin_time` date DEFAULT NULL COMMENT '生产日期',
  `end_time` date DEFAULT NULL COMMENT '有效期',
  `unit_price` decimal(18,6) DEFAULT NULL COMMENT '单价快照',
  `qty_in_package` decimal(18,6) DEFAULT NULL COMMENT '本包对应耗材数量（定数含量）',
  `package_index` int DEFAULT NULL COMMENT '同一明细行内包序号',
  `package_total` int DEFAULT NULL COMMENT '该明细行生成总包数',
  `in_hospital_code` varchar(200) NOT NULL COMMENT '院内码（打印主展示）',
  `fixed_package_barcode` varchar(200) DEFAULT NULL COMMENT '定数包条码（可与院内码一致或扩展码制）',
  `stk_inventory_id` varchar(64) DEFAULT NULL COMMENT '关联仓库库存 stk_inventory.id',
  `stk_dep_inventory_id` varchar(64) DEFAULT NULL COMMENT '关联科室库存 stk_dep_inventory.id',
  `hc_barcode_master_id` varchar(36) DEFAULT NULL COMMENT '归属主档 hc_barcode_master.id',
  `audit_by` varchar(64) DEFAULT NULL COMMENT '审核生成操作人',
  `audit_time` datetime DEFAULT NULL COMMENT '审核生成时间',
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
  `del_flag` tinyint NOT NULL DEFAULT 0 COMMENT '删除标志 0正常 1删除',
  `create_by` varchar(64) DEFAULT NULL COMMENT '创建者',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  ...
  `delete_time` datetime DEFAULT NULL COMMENT '删除时间',
  PRIMARY KEY (`id`),
  KEY `idx_stk_lv_io_bar_tenant_io` (`tenant_id`,`io_direction`),
  KEY `idx_stk_lv_io_bar_bill` (`tenant_id`,`stk_io_bill_id`),
  KEY `idx_stk_lv_io_bar_entry` (`tenant_id`,`stk_io_bill_entry_id`),
  KEY `idx_stk_lv_io_bar_code` (`tenant_id`,`in_hospital_code`),
  KEY `idx_stk_lv_io_bar_pkg` (`tenant_id`,`fixed_package_barcode`),
  KEY `idx_stk_lv_io_bar_master` (`hc_barcode_master_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='低值出入库定数包/院内码明细（入库审核/出库审核生成，一行一包）';
/

-- 高低值统一：条码归属主档（一条码一行，便于临床/仓库查询来源业务）
CREATE TABLE IF NOT EXISTS `hc_barcode_master` (
  `id` varchar(36) NOT NULL COMMENT '主键UUID7',
  `tenant_id` varchar(36) NOT NULL COMMENT '租户ID',
  `barcode_value` varchar(200) NOT NULL COMMENT '条码值（院内码或定数包条码，唯一性由业务+租户保证）',
  `value_level` char(1) NOT NULL COMMENT '1高值 2低值',
  `business_type_code` varchar(64) NOT NULL COMMENT '业务类型编码：如 LV_STK_IN_AUDIT、LV_STK_OUT_AUDIT、GZ_ORDER_PREP_ACCEPT',
  `business_type_name` varchar(128) DEFAULT NULL COMMENT '业务类型名称（展示）',
  `source_table` varchar(64) DEFAULT NULL COMMENT '来源表名：stk_lv_io_inhospital_barcode / gz_order_entry_inhospitalcode_list',
  `source_row_id` varchar(64) DEFAULT NULL COMMENT '来源表主键字符串',
  `bill_domain` varchar(32) DEFAULT NULL COMMENT '单据域：STK_IO_BILL / GZ_ORDER / GZ_REFUND 等',
  `bill_id` varchar(64) DEFAULT NULL COMMENT '业务单据主表ID',
  `bill_no` varchar(128) DEFAULT NULL COMMENT '业务单号快照',
  `bill_type` varchar(32) DEFAULT NULL COMMENT '单据类型快照',
  `bill_entry_id` varchar(64) DEFAULT NULL COMMENT '业务单据明细ID',
  ...
  `material_model` varchar(256) DEFAULT NULL COMMENT '型号快照',
  `material_unit_name` varchar(64) DEFAULT NULL COMMENT '单位名称快照',
  `factory_id` varchar(64) DEFAULT NULL COMMENT '厂家ID快照',
  `factory_name` varchar(256) DEFAULT NULL COMMENT '厂家名称快照',
  `supplier_id` varchar(64) DEFAULT NULL COMMENT '供应商ID快照',
  `supplier_name` varchar(256) DEFAULT NULL COMMENT '供应商名称快照',
  `warehouse_id` varchar(64) DEFAULT NULL COMMENT '归属仓库ID快照',
  `warehouse_name` varchar(200) DEFAULT NULL COMMENT '仓库名称快照',
  `department_id` varchar(64) DEFAULT NULL COMMENT '归属科室ID快照',
  `department_name` varchar(200) DEFAULT NULL COMMENT '科室名称快照',
  `batch_no` varchar(100) DEFAULT NULL COMMENT '批次号',
  `batch_number` varchar(100) DEFAULT NULL COMMENT '批号',
  `begin_time` date DEFAULT NULL COMMENT '生产日期',
  `end_time` date DEFAULT NULL COMMENT '有效期',
  `master_barcode` varchar(200) DEFAULT NULL COMMENT '高值主条码快照',
  `secondary_barcode` varchar(200) DEFAULT NULL COMMENT '高值辅条码快照',
  `fixed_package_qty` decimal(18,6) DEFAULT NULL COMMENT '定数包含量（低值）',
  `current_holder_type` varchar(16) DEFAULT NULL COMMENT '当前持有方：WH仓库/DEPT科室/UNKNOWN',
  `current_warehouse_id` varchar(64) DEFAULT NULL COMMENT '当前所在仓库',
  `current_department_id` varchar(64) DEFAULT NULL COMMENT '当前所在科室',
  `status` varchar(32) DEFAULT 'ACTIVE' COMMENT 'ACTIVE有效/CONSUMED消耗/CANCELLED作废等',
  `del_flag` tinyint NOT NULL DEFAULT 0 COMMENT '删除标志',
  `create_by` varchar(64) DEFAULT NULL COMMENT '创建者',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT NULL COMMENT '更新者',
  ...
  `delete_time` datetime DEFAULT NULL COMMENT '删除时间',
  PRIMARY KEY (`id`),
  KEY `idx_hc_barcode_master_tenant_code` (`tenant_id`,`barcode_value`),
  KEY `idx_hc_barcode_master_type` (`tenant_id`,`business_type_code`),
  KEY `idx_hc_barcode_master_bill` (`tenant_id`,`bill_domain`,`bill_id`),
  KEY `idx_hc_barcode_master_mat` (`tenant_id`,`material_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='高低值条码归属主档（统一查询入口）';
/

-- 高低值统一：条码流通事件（入库/出库/备货验收/退库/退货等写入流水）
CREATE TABLE IF NOT EXISTS `hc_barcode_flow` (
  `id` varchar(36) NOT NULL COMMENT '主键UUID7',
  `tenant_id` varchar(36) NOT NULL COMMENT '租户ID',
  `hc_barcode_master_id` varchar(36) NOT NULL COMMENT '归属主档ID',
  `barcode_value` varchar(200) NOT NULL COMMENT '条码冗余（免关联即可列表打印）',
  `value_level` char(1) NOT NULL COMMENT '1高值 2低值',
  `seq_no` int DEFAULT 1 COMMENT '同一条码事件顺序号',
  `event_code` varchar(64) NOT NULL COMMENT '事件编码：LV_IN_AUDIT/LV_OUT_AUDIT/GZ_PREP_IN/CK/TK/TH 等',
  `event_name` varchar(128) DEFAULT NULL COMMENT '事件名称（中文展示）',
  `event_time` datetime NOT NULL COMMENT '事件发生时间',
  `bill_domain` varchar(32) DEFAULT NULL COMMENT '单据域',
  `bill_id` varchar(64) DEFAULT NULL COMMENT '单据主表ID',
  `bill_no` varchar(128) DEFAULT NULL COMMENT '单号快照',
  `bill_entry_id` varchar(64) DEFAULT NULL COMMENT '单据明细ID',
  `from_warehouse_id` varchar(64) DEFAULT NULL COMMENT '流出仓库',
  `to_warehouse_id` varchar(64) DEFAULT NULL COMMENT '流入仓库',
  `from_department_id` varchar(64) DEFAULT NULL COMMENT '流出科室',
  `to_department_id` varchar(64) DEFAULT NULL COMMENT '流入科室',
  `qty` decimal(18,6) DEFAULT NULL COMMENT '数量快照',
  `material_name` varchar(256) DEFAULT NULL COMMENT '品名快照',
  `material_speci` varchar(256) DEFAULT NULL COMMENT '规格快照',
  `material_model` varchar(256) DEFAULT NULL COMMENT '型号快照',
  `operator_id` varchar(64) DEFAULT NULL COMMENT '操作人ID',
  `operator_name` varchar(128) DEFAULT NULL COMMENT '操作人姓名快照',
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
  `del_flag` tinyint NOT NULL DEFAULT 0 COMMENT '删除标志',
  `create_by` varchar(64) DEFAULT NULL COMMENT '创建者',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  ...
  `delete_time` datetime DEFAULT NULL COMMENT '删除时间',
  PRIMARY KEY (`id`),
  KEY `idx_hc_barcode_flow_master` (`tenant_id`,`hc_barcode_master_id`),
  KEY `idx_hc_barcode_flow_code` (`tenant_id`,`barcode_value`),
  KEY `idx_hc_barcode_flow_time` (`tenant_id`,`event_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='高低值条码流通记录';
/

-- 高值：仓库流水（与 t_hc_ck_flow 并存，专记高值备货/验收/出入库等，便于独立查询与打印快照）
CREATE TABLE IF NOT EXISTS `gz_wh_flow` (
  `id` varchar(36) NOT NULL COMMENT '主键UUID7',
  `tenant_id` varchar(36) NOT NULL COMMENT '租户ID',
  `bill_id` varchar(64) DEFAULT NULL COMMENT '来源单据主表ID',
  `bill_no` varchar(128) DEFAULT NULL COMMENT '单号快照',
  `entry_id` varchar(64) DEFAULT NULL COMMENT '来源单据明细ID',
  `ref_bill_id` varchar(36) DEFAULT NULL COMMENT '关联主单UUID（若有）',
  ...
  `batch_no` varchar(100) DEFAULT NULL COMMENT '批次号',
  `batch_number` varchar(100) DEFAULT NULL COMMENT '批号',
  `batch_id` varchar(64) DEFAULT NULL COMMENT '批次对象 stk_batch.id',
  `qty` decimal(18,6) DEFAULT NULL COMMENT '数量',
  `unit_price` decimal(18,6) DEFAULT NULL COMMENT '单价',
  `amt` decimal(18,6) DEFAULT NULL COMMENT '金额',
  `begin_time` date DEFAULT NULL COMMENT '生产日期',
  ...
  `supplier_name` varchar(256) DEFAULT NULL COMMENT '供应商名称快照',
  `factory_id` varchar(64) DEFAULT NULL COMMENT '厂家ID',
  `factory_name` varchar(256) DEFAULT NULL COMMENT '厂家名称快照',
  `gz_depot_inventory_id` varchar(64) DEFAULT NULL COMMENT '高值仓库库存行 gz_depot_inventory.id',
  `in_hospital_code` varchar(200) DEFAULT NULL COMMENT '院内码',
  `master_barcode` varchar(200) DEFAULT NULL COMMENT '主条码',
  `secondary_barcode` varchar(200) DEFAULT NULL COMMENT '辅条码',
  `lx` varchar(16) DEFAULT NULL COMMENT 'RK/CK/TH/TK 等',
  `flow_time` datetime DEFAULT NULL COMMENT '流水时间',
  `origin_business_type` varchar(64) DEFAULT NULL COMMENT '来源业务类型中文',
  `del_flag` tinyint NOT NULL DEFAULT 0 COMMENT '删除标志',
  `create_by` varchar(64) DEFAULT NULL COMMENT '创建者',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` varchar(64) DEFAULT NULL COMMENT '更新者',
...
CREATE TABLE IF NOT EXISTS `gz_dep_flow` (
  `id` varchar(36) NOT NULL COMMENT '主键UUID7',
  `tenant_id` varchar(36) NOT NULL COMMENT '租户ID',
  `bill_id` varchar(64) DEFAULT NULL COMMENT '来源单据主表ID',
  `bill_no` varchar(128) DEFAULT NULL COMMENT '单号快照',
  ...
  `department_id` varchar(64) DEFAULT NULL COMMENT '科室ID',
  `department_name` varchar(200) DEFAULT NULL COMMENT '科室名称快照',
  `warehouse_id` varchar(64) DEFAULT NULL COMMENT '归属仓库ID',
  `warehouse_name` varchar(200) DEFAULT NULL COMMENT '仓库名称快照',
  `material_id` varchar(64) DEFAULT NULL COMMENT '耗材ID',
  `material_code` varchar(128) DEFAULT NULL COMMENT '耗材编码快照',
  `material_name` varchar(256) DEFAULT NULL COMMENT '产品名称快照',
  ...
  `factory_id` varchar(64) DEFAULT NULL COMMENT '厂家ID',
  `factory_name` varchar(256) DEFAULT NULL COMMENT '厂家名称快照',
  `gz_dep_inventory_id` varchar(64) DEFAULT NULL COMMENT '高值科室库存行 gz_dep_inventory.id',
  `in_hospital_code` varchar(200) DEFAULT NULL COMMENT '院内码',
  `master_barcode` varchar(200) DEFAULT NULL COMMENT '主条码',
  `secondary_barcode` varchar(200) DEFAULT NULL COMMENT '辅条码',
  `lx` varchar(16) DEFAULT NULL COMMENT 'CK/TK 等',
  `flow_time` datetime DEFAULT NULL COMMENT '流水时间',
  `origin_business_type` varchar(64) DEFAULT NULL COMMENT '来源业务类型中文',
  `del_flag` tinyint NOT NULL DEFAULT 0 COMMENT '删除标志',
  `create_by` varchar(64) DEFAULT NULL COMMENT '创建者',
