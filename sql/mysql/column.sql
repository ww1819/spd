--mysql 追加字段
-- 临时修改语句结束符（避免存储过程内;提前终止定义）
DELIMITER //

-- 核心语法：CREATE PROCEDURE IF NOT EXISTS → 仅不存在时创建，存在则不执行任何操作（无报错）
CREATE PROCEDURE IF NOT EXISTS `add_table_column`(
    IN p_table_name VARCHAR(64),  -- 表名（MySQL标识符最大64字符）
    IN p_column_name VARCHAR(64), -- 字段名
    IN p_column_type VARCHAR(64), -- 字段类型（如VARCHAR(50)、INT、DATETIME、DECIMAL(10,2)）
    IN p_column_comment VARCHAR(256), -- 字段注释
    IN p_default_value VARCHAR(256) -- 可选默认值，传NULL则不设置默认值
)
add_column_block:
BEGIN
...
END IF;
    ...
    -- 4. 预处理并执行动态SQL（MySQL标准方式，避免语法解析问题）
...
/**
    ...
    IN p_default_value VARCHAR(256) -- 可选入参：字段默认值
 */
