...

{
     ...
     * 使用追溯汇总表（执行科室）：按执行科室+耗材汇总数量金额

     */

    @PreAuthorize("@ss.hasPermi('gz:traceability:list')")

    @GetMapping("/summary/execDept/list")

    public TableDataInfo summaryExecDeptList(GzTraceability gzTraceability)

    {

        startPage();

        List<com.spd.gz.domain.GzTraceabilitySummaryVo> list =
     ...
     * 使用追溯汇总表（开单科室）：按开单科室+耗材汇总数量金额

     */

    @PreAuthorize("@ss.hasPermi('gz:traceability:list')")

    @GetMapping("/summary/applyDept/list")

    public TableDataInfo summaryApplyDeptList(GzTraceability gzTraceability)
     ...
     * 使用追溯汇总表（供应商）：按供应商+耗材汇总数量金额

     */

    @PreAuthorize("@ss.hasPermi('gz:traceability:list')")

    @GetMapping("/summary/supplier/list")

    public TableDataInfo summarySupplierList(GzTraceability gzTraceability)
