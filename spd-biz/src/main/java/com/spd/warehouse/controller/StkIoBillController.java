...
public class StkIoBillController extends BaseController
{
    ...
    {
        // 合计必须在 startPage 之前查，避免 PageHelper 分页/偏移把聚合结果挤成空行导致 totalAmt=0
        clearPage();
     ...
     * @param yearMonth 统计月 yyyy-MM，空则当月
    ...
    public AjaxResult outboundSummaryByDepartment(String yearMonth) {
        List<Map<String, Object>> list = stkIoBillService.selectOutboundSummaryByDepartment(yearMonth);
        return success(list);
    }
    ...
    /**
     * 数据可视化大屏：指定月送货入库（已审核入退货）按供应商汇总配送金额，取前 10，按金额降序。
     * @param yearMonth 统计月 yyyy-MM，空则当月
     */
    @GetMapping("/biScreenInboundSupplierTop10")
    public AjaxResult biScreenInboundSupplierTop10(String yearMonth) {
        List<Map<String, Object>> list = stkIoBillService.selectBiScreenInboundSupplierTop10(yearMonth);
        return success(list != null ? list : new java.util.ArrayList<>());
    }

    /**
     * 数据可视化大屏：指定月已审核入退货按日汇总金额（高值耗材 is_gz=1 / 低值其余），用于折线+柱状图。
     * @param yearMonth 统计月 yyyy-MM，空则当月
     */
    @GetMapping("/biScreenInboundDailyHighLowValue")
    public AjaxResult biScreenInboundDailyHighLowValue(String yearMonth) {
        List<Map<String, Object>> list = stkIoBillService.selectBiScreenInboundDailyHighLowValue(yearMonth);
        return success(list != null ? list : new java.util.ArrayList<>());
    }

    /**
     * 数据可视化大屏：指定月已审核出退库按单个耗材汇总出库金额，TOP20，金额降序（耗材排行榜）。
     * @param yearMonth 统计月 yyyy-MM，空则当月
     */
    @GetMapping("/biScreenOutboundMaterialMonthTop")
    public AjaxResult biScreenOutboundMaterialMonthTop(String yearMonth) {
        List<Map<String, Object>> list = stkIoBillService.selectBiScreenOutboundMaterialMonthTop(yearMonth);
        return success(list != null ? list : new java.util.ArrayList<>());
    }

    /**
     * 数据可视化大屏：指定月已审核入退货按耗材「财务分类」汇总入库金额（退库冲减），按金额降序。
     * @param yearMonth 统计月 yyyy-MM，空则当月
     */
    @GetMapping("/biScreenInboundFinanceCategoryMonth")
    public AjaxResult biScreenInboundFinanceCategoryMonth(String yearMonth) {
        List<Map<String, Object>> list = stkIoBillService.selectBiScreenInboundFinanceCategoryMonth(yearMonth);
        return success(list != null ? list : new java.util.ArrayList<>());
    }

    /**
     * 数据可视化大屏：出库单（201）/入库单（101）笔数。
     * 统计月为当月时按今日；历史月按该月整月。
     * @param yearMonth 统计月 yyyy-MM，空则当月
     */
    @GetMapping("/biScreenTodayInboundOutboundBillCount")
    public AjaxResult biScreenTodayInboundOutboundBillCount(String yearMonth) {
        Map<String, Object> row = stkIoBillService.selectBiScreenTodayInboundOutboundBillCount(yearMonth);
        return success(row != null ? row : new java.util.HashMap<>());
    }

    /**
     * 数据可视化大屏：指定年已审核入退货按自然月汇总金额（入库 101、退货入库 301），用于年度采购/入退货曲线。
     * @param year 统计年，空则当年；也可传 yearMonth（取年份）
     */
    @GetMapping("/biScreenYearInboundReturnByMonth")
    public AjaxResult biScreenYearInboundReturnByMonth(Integer year, String yearMonth) {
        Integer statYear = year;
        if (statYear == null && StringUtils.isNotEmpty(yearMonth) && yearMonth.length() >= 4) {
            try {
                statYear = Integer.parseInt(yearMonth.substring(0, 4));
            } catch (NumberFormatException ignored) {
                statYear = null;
            }
        }
        List<Map<String, Object>> list = stkIoBillService.selectBiScreenYearInboundReturnByMonth(statYear);
        return success(list != null ? list : new java.util.ArrayList<>());
    }
