...
//    public int deleteStkIoBillByIds(Long[] ids);
     ...
     */
    List<Map<String, Object>> selectOutboundSummaryByDepartment(String yearMonth);

    /** 大屏：指定月送货入库前十供应商（已审核入退货按供应商汇总金额降序） */
    List<Map<String, Object>> selectBiScreenInboundSupplierTop10(String yearMonth);

    /** 大屏：指定月入退货按日汇总金额（高值 is_gz=1 / 低值其余） */
    List<Map<String, Object>> selectBiScreenInboundDailyHighLowValue(String yearMonth);

    /** 大屏：指定月出退库按单个耗材汇总金额 TOP20（降序） */
    List<Map<String, Object>> selectBiScreenOutboundMaterialMonthTop(String yearMonth);

    /** 大屏：指定月入退货按财务分类汇总入库金额（降序） */
    List<Map<String, Object>> selectBiScreenInboundFinanceCategoryMonth(String yearMonth);

    /** 首页/大屏：指定月出退库按财务分类汇总出库金额（降序） */
    List<Map<String, Object>> selectBiScreenOutboundFinanceCategoryMonth(String yearMonth);

    /** 大屏：出库/入库单据笔数（当月按今日，历史月按整月） */
    Map<String, Object> selectBiScreenTodayInboundOutboundBillCount(String yearMonth);

    /** 大屏：指定年按自然月汇总入库金额(101)、退货入库金额(301) */
    List<Map<String, Object>> selectBiScreenYearInboundReturnByMonth(Integer statYear);
